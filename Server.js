...
function serveStatic(req, res, pathname) {
  ...
  if (pathname === "/" || pathname === "") {
    ...
    const rootFile = path.join(__dirname, safe);
    const webFile = path.join(__dirname, "web", safe);

    if (fs.existsSync(rootFile) && fs.statSync(rootFile).isFile()) {
      filePath = rootFile;
    } else if (fs.existsSync(webFile) && fs.statSync(webFile).isFile()) {
      filePath = webFile;
    }
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (
    ...
    try {
      ...
      const player = {
        ...
        name: cleanName(
          body.name || body.player || body.profile,
          "Jugador 1"
        ),
        profile: cleanName(
          body.profile || body.name || body.player,
          "Mirella"
        )
      };
  ...
  if (
    ...
    try {
      ...
      const message = {
        id: crypto.randomUUID(),
        from: body.from || body.player || null,
        text: String(
          body.text ||
          body.message ||
          ""
